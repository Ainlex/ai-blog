...
export default function SidebarCategories({ onClick }: { onClick?: () => void }) {
  ...
  useEffect(() => {
    fetch('/api/categorias-extra-counts')
      .then(res => res.json())
      .then(data => {
        if (data.success) setCategoryCounts(data.counts)
      })
  }, [])
  return (
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-2">
        {CATEGORIES.map((category) => (
          <Link
            ...
            onClick={onClick}
            className="flex items-center gap-3 px-5 py-3 rounded-lg hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors text-base font-medium border border-gray-100 dark:border-gray-800 mb-2"
          >
            ...
            <span className="text-xs text-gray-500 ml-2">{categoryCounts[category.name] || 0} artículos</span>
          </Link>
        ))}
      </nav>
    </div>
